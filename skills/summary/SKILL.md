...
**Constraints.** No paragraph longer than two sentences. Bullets under ten words. Must survive Print to PDF — no fixed viewport heights, nothing that only appears on scroll. Light by default; add a dark variant only when the page will be viewed inside a container with its own theme setting.
